...
int main(int argc,char *argv[])
{
	...
	while (1)
	{
		...
		printf("The smoker %d who owned tobacoo sent a request to the providers.\n",getpid());
		sleep(rate);
		msg_buf.mtype=4;
		msgrcv(mesg_id,&msg_buf,1,4,0);
		printf("The smoker %d who owned tobacoo got all ingredients from the provider %d.\n",getpid(),msg_buf.mtext[0]);
		printf("The smoker %d who owned tobacoo just smoked.\n",getpid());
		up(smok1_sem);
	}
return EXIT_SUCCESS;
